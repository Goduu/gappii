...
export const validateTranslateLessonApiResponse = (response: object | null, newLanguage: SupportedLanguage, onError: (error: string) => void): ApiActivityResponse | null => {
    if (!response) {
    ...
    if (!data.language) {
        ...
        return null
    } else if (data.language !== newLanguage) {
        onError("Invalid response from API, language set wrong");
        return null
    }
